...
def handle_grasp(req):
    ...
    try:
        # Approach the object
        approach_pose = [req.x, req.y, req.z + 0.1, req.roll, req.pitch, req.yaw]
        n.set_arm_max_velocity(10)
        n.move_pose(*approach_pose)
        rospy.sleep(1)

        # Adjust the gripper's orientation
        n.move_pose(req.x, req.y, req.z + 0.05, req.roll, req.pitch, req.yaw)
        rospy.sleep(1)

        # Open gripper 
        n.change_tool(TOOL_GRIPPER_4_ID)
        n.open_gripper(TOOL_GRIPPER_4_ID,200)

        # Grasp the object
        n.move_pose(req.x, req.y, req.z, req.roll, req.pitch, req.yaw)
        rospy.sleep(1)
        n.close_gripper(TOOL_GRIPPER_4_ID,200)
